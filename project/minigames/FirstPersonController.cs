...
public partial class FirstPersonController : CharacterBody3D
{
    ...
    {
        if(IsInstanceValid(GetNode<FirstPersonCamera>("Node3D")))
		{
			firstPersonCamera = GetNode<FirstPersonCamera>("Node3D");
		}
	...
	private void GetInput()
	{
		...
		if(Input.IsActionJustPressed("meow"))
		{
			Meow();
		}
	}
	...
	private void Move()
	{
		...
		{
			velocity.X = smoothedDirection.X * Speedo;
			velocity.Z = smoothedDirection.Z * Speedo;
		}
		else
		{
			velocity.X = Mathf.MoveToward(Velocity.X, 0, Speedo);
			velocity.Z = Mathf.MoveToward(Velocity.Z, 0, Speedo);
		}
		Velocity = velocity;
		MoveAndSlide();
